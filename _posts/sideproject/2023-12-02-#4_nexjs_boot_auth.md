...
기존의 프로젝트는 Spring Boot 단일 어플리케이션이었습니다. 템플릿 엔진으로 타임리프를 사용하여 SSR 방식으로 뷰 페이지까지 제공했습니다. Next.js 사용하여 프론트 서버를 도입하고 기존의 Springboot는 REST API 서버로 마이그레이션을 진행하고 있습니다. 이에 따라 기존의 인증 방식을 새롭게 변경해야 했습니다. 
...
기존의 인증 방식은 스프링 시큐리티를 이용하여 Oauth2을 이용한 SNS 로그인, 애플리케이션 레벨의 자체 로그인을 구현하여 사용했습니다. 
...
스프링 시큐리티로 Oauth2 인증을 처리하기 위해서는 사전 설정이 필요합니다. 소셜 로그인 페이지를 제공하기 위한 설정, 각 소셜 로그인을 이용하기 위한 API 키 설정 등등이 필요합니다. 나아가 스프링 시큐리티 내부적으로 복잡한 인증 Flow로 인증을 처리합니다. 
...
우선 시퀀스 다이어그램을 그려보며 큰 흐름을 이해하기로 했습니다. 아래 시퀀스 다이어그램은 고민과 시행착오를 거쳐 완성된 현재 인증 방식 Flow입니다.
...
아래에서 NEXT.js 와 Spring Boot 코드로 좀 더 자세하게 알아보겠습니다.

<br><br>

# NEXT.js

NEXT.js 구체적인 흐름은 빠른 시일 내에 업로드 됩니다. 😀
...
인증의 한 부분인 로그인의 과정이 끝났습니다. 이제 stateless한 인증의 이점을 살리기 위해 로그인 이후의 **검증과 인가**에 대한 전략을 어떻게 세울 것인가 고민이 필요합니다. 

저희가 사용한 방법을 다음 게시글에서 정리할 예정입니다.